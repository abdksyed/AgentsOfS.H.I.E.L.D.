document.addEventListener('DOMContentLoaded', () => {
     ...
     */
    // Define an interface for the recorded item structure if possible, use 'any' for now
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', () => {
            ...
            chrome.runtime.sendMessage({ action: 'downloadEventsAction' }, (response?: DownloadEventsResponse) => {
                // Define expected state type
                 interface ExtensionState {
                    isRecording: boolean;
                    recordedData?: Array<any>;
                    activeTabId?: number | null;
                    isScreenRecording: boolean;
                    hasVideo?: boolean;
                 }
        ...
        downloadVideoBtn.addEventListener('click', () => {
            ...
            chrome.runtime.sendMessage({ action: 'downloadVideoAction' }, (response?: DownloadVideoResponse) => {
                 const reEnableButton = () => {
                     ...
                     chrome.runtime.sendMessage({ action: 'getInitialState' }, (state?: ExtensionState) => {
                         if (state && downloadVideoBtn) {
                            downloadVideoBtn.disabled = !state.hasVideo || state.isScreenRecording || state.isRecording;
                         } else if(downloadVideoBtn){
                              downloadVideoBtn.disabled = true; // Disable if state is unavailable
                         }
                     });
                 };
